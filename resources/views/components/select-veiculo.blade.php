...
<<<<<<< HEAD
            <option value="{{ $item->id }}" selected>Veículo: {{ $item->placa }}{{ (count($item->reboque)!=0)? ' - Semireboque '.$item->reboque->first()->placa:''  }}</option>
            @else
                <option value="{{ $item->id }}">Veículo: {{ $item->placa }} {{ (count($item->reboque)!=0)?' - Semireboque '.$item->reboque->first()->placa:'' }}</option>
=======
